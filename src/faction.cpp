...
// Pick an overall goal.
	goal = FactionGoal(rng(1, NUM_FACGOALS - 1));
	if (one_in(4))
	...
	job1 = FactionJob(rng(1, NUM_FACJOBS - 1));
	do
		job2 = FactionJob(rng(0, NUM_FACJOBS - 1));
	while (job2 == job1);
	...
	do {
		...
		if (!has_value(FactionValue(v)) && matches_us(FactionValue(v))) {
			values |= mfb(v);
			tries = 0;
			num_values++;
...
bool Faction::has_job(FactionJob j) {
	return (job1 == j || job2 == j);
}

bool Faction::has_value(FactionValue v) {
	return values & mfb(v);
}

bool Faction::matches_us(FactionValue v) {
	int numvals = 2;
	if (job2 != FACJOB_NULL)
		numvals++;
	for (int i = 0; i < NUM_FACVALS; i++) {
		if (has_value(FactionValue(i)))
			numvals++;
	}
