...
bool is_river(overmap_terrain_id ter) {
	if (ter == ot_null || (ter >= ot_bridge_ns && ter <= ot_river_nw))
		return true;
	return false;
}

bool is_ground(overmap_terrain_id ter) {
	if (ter <= ot_road_nesw_manhole)
		return true;
	return false;
}

bool is_wall_material(overmap_terrain_id ter) {
	if (is_ground(ter) ||
...
overmap_terrain_id shop(int dir) {
	overmap_terrain_id ret = ot_s_lot;
	switch (rng(0, 10)) {
		case 0:
			ret = ot_s_lot;
			break;
		case 1:
			...
			break;
		case 2:
			ret = ot_s_pharm_north;
			break;
		case 3:
			ret = ot_s_grocery_north;
			break;
		case 4:
			ret = ot_s_hardware_north;
			break;
		case 5:
			ret = ot_s_sports_north;
			break;
		case 6:
			ret = ot_s_liquor_north;
			break;
		case 7:
			ret = ot_s_gun_north;
			break;
		case 8:
			ret = ot_s_clothes_north;
			break;
		case 9:
			ret = ot_s_library_north;
			break;
		case 10:
			ret = ot_sub_station_north;
			break;
	}
	if (ret == ot_s_lot)
		return ret;
	...
	switch (dir) {
		case 0:
			break;
		case 1:
			ret = overmap_terrain_id(ret + 1);
			break;
		case 2:
			ret = overmap_terrain_id(ret + 2);
			break;
		case 3:
			ret = overmap_terrain_id(ret + 3);
			break;
		default:
...
overmap_terrain_id house(int dir) {
	bool base = one_in(30);
	if (dir < 0) dir += 4;
	switch (dir) {
		case 0:
			return base ? ot_house_base_north : ot_house_north;
		case 1:
			return base ? ot_house_base_east : ot_house_east;
		case 2:
			return base ? ot_house_base_south : ot_house_south;
		case 3:
			return base ? ot_house_base_west : ot_house_west;
		default:
			debugmsg("Bad rotation of house.");
			return ot_null;
	}
}
...
overmap_terrain_id &overmap::ter(int x, int y) {
...
Point overmap::find_closest(Point origin, overmap_terrain_id type, int type_range, int &dist, bool must_be_seen) {
	int max = dist;
...
void overmap::place_settlements(game *g) {
	...
	for (int i = 0; i < towns.size(); i++) {
		...
		switch (rng(0, 3)) {
			case 0:
				ter(towns[i].posx, towns[i].posy - towns[i].size) = ot_gate;
				break;
			case 1:
				ter(towns[i].posx + towns[i].size, towns[i].posy) = ot_gate;
				break;
			case 2:
				ter(towns[i].posx, towns[i].posy + towns[i].size) = ot_gate;
				break;
...
void overmap::settlement_building(settlement &set, int x, int y) {
	std::vector <overmap_terrain_id> valid;
	...
	overmap_terrain_id pick = valid[rng(0, valid.size() - 1)];
	ter(x, y) = pick;
...
void overmap::make_hiway(int x1, int y1, int x2, int y2, overmap_terrain_id base) {
...
void overmap::place_hiways(std::vector <city> cities, overmap_terrain_id base) {
	if (cities.size() == 1)
		return;
	city best;
	int closest = -1;
...
void overmap::polish(overmap_terrain_id min, overmap_terrain_id max) {
...
bool overmap::is_road(overmap_terrain_id base, int x, int y) {
	overmap_terrain_id min, max;
...
void overmap::good_road(overmap_terrain_id base, int x, int y) {
	int d = ot_road_ns;
	if (is_road(base, x, y - 1)) {
		if (is_road(base, x + 1, y)) {
			if (is_road(base, x, y + 1)) {
				if (is_road(base, x - 1, y))
					ter(x, y) = overmap_terrain_id(base + ot_road_nesw - d);
				else
					ter(x, y) = overmap_terrain_id(base + ot_road_nes - d);
			}
			else {
				if (is_road(base, x - 1, y))
					ter(x, y) = overmap_terrain_id(base + ot_road_new - d);
				else
					ter(x, y) = overmap_terrain_id(base + ot_road_ne - d);
			}
		}
		else {
			if (is_road(base, x, y + 1)) {
				if (is_road(base, x - 1, y))
					ter(x, y) = overmap_terrain_id(base + ot_road_nsw - d);
				else
					ter(x, y) = overmap_terrain_id(base + ot_road_ns - d);
			}
			else {
				if (is_road(base, x - 1, y))
					ter(x, y) = overmap_terrain_id(base + ot_road_wn - d);
				else
					ter(x, y) = overmap_terrain_id(base + ot_road_ns - d);
			}
		}
		...
		if (is_road(base, x + 1, y)) {
			if (is_road(base, x, y + 1)) {
				if (is_road(base, x - 1, y))
					ter(x, y) = overmap_terrain_id(base + ot_road_esw - d);
				else
					ter(x, y) = overmap_terrain_id(base + ot_road_es - d);
			}
			else
				ter(x, y) = overmap_terrain_id(base + ot_road_ew - d);
		}
		else {
			if (is_road(base, x, y + 1)) {
				if (is_road(base, x - 1, y))
					ter(x, y) = overmap_terrain_id(base + ot_road_sw - d);
				else
					ter(x, y) = overmap_terrain_id(base + ot_road_ns - d);
			}
			else {
...
void overmap::open(game *g, int x, int y, int z) {
	...
	if (fin.is_open()) {
		for (int j = 0; j < OMAPY; j++) {
			...
			for (int i = 0; i < OMAPX; i++)
				ter(i, j) = overmap_terrain_id(fin.get() - 32);
		}
		for (int j = 0; j < OMAPY; j++) {
			fin.getline(line, OMAPX + 1);
			for (int i = 0; i < OMAPX; i++) {
