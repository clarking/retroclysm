...
// Data Initialization
	...
	void pick_recipes(std::vector<recipe *> &current, std::vector<bool> &available, craft_cat tab);// crafting.cpp
	...
	std::vector <Point> target(int &x, int &y, int lowx, int lowy, int hix, int hiy, std::vector <monster> t, int &target, item *relevent);
	...
	overmap_terrain_id ter_at(int x, int y, bool &mark_as_seen);
