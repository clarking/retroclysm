...
void game::init_recipes() {
	...
	// A recipe will not appear in your menu until your level in the primary skill is at least equal to the difficulty.
	// At that Point, your chance of success is still not great; a good 25% improvement over the difficulty is important

	RECIPE(itm_spear_wood, CC_WEAPON, sk_null, sk_null, 0, 800);
	TOOL(itm_hatchet, -1, itm_knife_steak, -1, itm_knife_butcher, -1, itm_knife_combat, -1, itm_machete, -1, NULL);
	...
	COMP(itm_shot_bird, 30, itm_shot_00, 18, itm_shot_slug, 15, itm_gasoline, 3, itm_grenade, 1, NULL);
	...
	TOOL(itm_screwdriver, -1, NULL);
	COMP(itm_flashlight, 1, itm_radio, 1, itm_two_way_radio, 1, itm_geiger, 1, itm_goggles_nv, 1, itm_transponder, 2, NULL);

	RECIPE(itm_power_supply, CC_ELECTRONIC, sk_electronics, sk_null, 2, 6500);
	TOOL(itm_screwdriver, -1, NULL);
	TOOL(itm_soldering_iron, 3, NULL);
	COMP(itm_amplifier, 2, itm_soldering_iron, 1, itm_electrohack, 1, itm_battery, 800, itm_geiger, 1, NULL);
	...
	COMP(itm_pan, 1, itm_pot, 1, itm_knife_butcher, 2, itm_knife_steak, 6, itm_knife_butter, 6, itm_muffler, 1, NULL);
