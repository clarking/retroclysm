...
game::game() {
	...
	for (int & kill : kills)    // Reset kill counts to 0
		kill = 0;
...
bool game::opening_screen() {
	...
	mvwprintz(w_open, 0, 1, c_blue, "Welcome to RetroClysm!");
	mvwprintz(w_open, 1, 0, c_red, "This alpha release is highly unstable. Please report any crashes or bugs to\nfivedozenwhales@gmail.com.");
	refresh();
	wrefresh(w_open);
	refresh();
	...
	do {
		if (layer == 1) {
			...
			if (sel1 == 1) {    // New Character
				...
				if (savegames.size() == 0)
					...
					for (int i = 0; i < savegames.size(); i++)
						mvwprintz(w_open, 5 + i, 12, (sel2 - 1 == i ? h_white : c_white), savegames[i].c_str());
				}
			...
			for (int i = 2; i < PLTYPE_MAX; i++)
				mvwprintz(w_open, 3 + i, 29, (sel1 == i ? h_white : c_white), pltype_name[i].c_str());
...
void game::update_skills() {
	...
	for (int i = 0; i < num_skill_types; i++) {
		...
		if (u.skexercise[i] < -100) {
			u.sklevel[i]--;
			add_msg("Your skill in %s has reduced to %d!", skill_name(skill(i)).c_str(), u.sklevel[i]);
			u.skexercise[i] = 0;
		}
		else if (u.skexercise[i] >= 100) {
			u.sklevel[i]++;
			add_msg("Your skill in %s has increased to %d!", skill_name(skill(i)).c_str(), u.sklevel[i]);
			u.skexercise[i] = 0;
		}
	}
}
...
void game::load(std::string name) {
	...
	fin >> turn >> tmptar >> tmprun >> mostseen >> nextinv >> nextspawn >> tmptemp >> levx >> levy >> levz >> comx >> comy;
			...
			for (int i = 0; i < num_ter_types; i++) {
				if (oterlist[i].name.find(term) != std::string::npos) {
					if (i == ot_forest || i == ot_hive || i == ot_hiway_ns || i == ot_bridge_ns)
						...
						range = 2;
					int maxdist = OMAPX;
					Point found = cur_om.find_closest(Point(cursx, cursy), overmap_terrain_id(i), range,
													  maxdist, true);
					i = num_ter_types;
					if (found.x != -1) {
						cursx = found.x;
...
void game::disp_NPCs() {
	...
	for (int i = 1; i < cur_om.npcs.size(); i++) {
		...
		else if (rl_dist(levx, levy, cur_om.npcs[i].mapx, cur_om.npcs[i].mapy) < rl_dist(levx, levy, closest[19]->mapx, closest[19]->mapy)) {
			for (int j = 0; j < 20; j++) {
				if (rl_dist(levx, levy, closest[j]->mapx, closest[j]->mapy) > rl_dist(levx, levy, cur_om.npcs[i].mapx, cur_om.npcs[i].mapy)) {
	...
	for (int i = 0; i < 20; i++)
		mvwprintz(w, i + 2, 0, c_white, "%s: %d:%d", closest[i]->name.c_str(), closest[i]->mapx, closest[i]->mapy);
...
Faction *game::list_factions(std::string title) {
	...
	do {
		...
		if (ch == 'j' || ch == 'k') {    // Changed our selection... update the windows
			...
			werase(w_info);
			// fac_*_text() is in faction.cpp
			mvwprintz(w_info, 0, 0, c_white, "Ranking: %s", fac_ranking_text(valfac[sel].likes_u).c_str());
			mvwprintz(w_info, 1, 0, c_white, "Respect: %s", fac_respect_text(valfac[sel].respects_u).c_str());
...
void game::draw() {
	...
	overmap_terrain_id cur_ter = cur_om.ter((levx + 1) / 2, (levy + 1) / 2);
...
bool game::u_see(monster *mon, int &t) {
	if (mon->has_flag(MF_DIGS) && !u.has_active_bionic(bio_ground_sonar) && rl_dist(u.posx, u.posy, mon->posx, mon->posy) > 1)
...
bool game::pl_sees(player *p, monster *mon, int &t) {
	if (mon->has_flag(MF_DIGS) && !p->has_active_bionic(bio_ground_sonar) && rl_dist(p->posx, p->posy, mon->posx, mon->posy) > 1)
		return false;    // Can't see digging monsters until we're right next to them
...
void game::monmove() {
	...
	for (int i = 0; i < z.size(); i++) {
		...
		if (dead)
			...
			if (u.has_active_bionic(bio_alarm) && u.power_level >= 1 && abs(z[i].posx - u.posx) <= 5 && abs(z[i].posy - u.posy) <= 5) {
				u.power_level--;
			...
			if (z[i].posx < 0 - SEEX * 3 || z[i].posy < 0 - SEEX * 3 || z[i].posx > SEEX * 6 || z[i].posy > SEEY * 6) {
...
void game::use_computer(int x, int y) {
	...
	int success = u.sklevel[sk_computer] - difficulty + int((u.int_cur - 8) / 2) + rng(0 - int(difficulty / 2), int(difficulty / 2));
...
void game::examine() {
	...
	if (m.has_flag(sealed, examx, examy)) {
		if (m.trans(examx, examy)) {
			...
			add_msg("There's something in there, but you can't see what it is, and the %s is firmly sealed.", m.tername(examx, examy).c_str());
		}
	}
	else {
		...
		if (m.i_at(examx, examy).size() == 0 && !(m.has_flag(swimmable, examx, examy) || m.ter(examx, examy) == t_toilet))
			add_msg("It is empty.");
		else
	...
	if (m.ter(examx, examy) == t_card_reader) {
		...
		bool using_electrohack = (u.has_amount(itm_electrohack, 1) && query_yn("Use electrohack on the reader?"));
		bool using_fingerhack = (!using_electrohack && u.has_bionic(bio_fingerhack) && u.power_level > 0 && query_yn("Use fingerhack on the reader?"));
		...
		switch (menu("Bulletin Board", "Check jobs", "Check events", "Check other notices", "Post notice", "Cancel", NULL)) {
			case 1:
				break;
			case 2:
				break;
...
void game::look_around() {
	...
	wborder(w_look, LINE_XOXO, LINE_XOXO, LINE_OXOX, LINE_OXOX, LINE_OXXO, LINE_OOXX, LINE_XXOO, LINE_XOOX);
	...
	do {
		...
		if (u_see(lx, ly, junk)) {
			if (m.move_cost(lx, ly) == 0)
				...
				mvwprintw(w_look, 1, 1, "%s; Movement cost %d", m.tername(lx, ly).c_str(), m.move_cost(lx, ly) * 50);

			mvwprintw(w_look, 2, 1, "%s", m.features(lx, ly).c_str());
			field tmpfield = m.field_at(lx, ly);

			if (tmpfield.type != fd_null)
				mvwprintz(w_look, 4, 1, fieldlist[tmpfield.type].color[tmpfield.density - 1], "%s", fieldlist[tmpfield.type].name[tmpfield.density - 1].c_str());

			if (m.tr_at(lx, ly) != tr_null && u.per_cur - u.encumb(bp_eyes) >= traps[m.tr_at(lx, ly)]->visibility)
				mvwprintz(w_look, 5, 1, traps[m.tr_at(lx, ly)]->color, "%s", traps[m.tr_at(lx, ly)]->name.c_str());
...
void game::pickup(int posx, int posy, int min) {
	...
	if (m.i_at(posx, posy).size() == 0) {
		if (m.has_flag(swimmable, posx, posy) || m.ter(posx, posy) == t_toilet) {
			...
			if (cont->type->id == itm_null)
				...
				if (!cont_type->flags & mfb(con_wtight)) {
					...
					if (query_yn("Can't transport water in a %s.  Drink it now?", cont->tname().c_str())) {
						cont->put_in(water);
		...
		else if (!u.is_armed() && (u.volume_carried() + newit.volume() > u.volume_capacity() - 2 || newit.is_weap())) {
			u.weapon = newit;
			m.i_clear(posx, posy);
			u.moves -= 100;
			add_msg("Wielding %c - %s", newit.invlet, newit.tname().c_str());
	...
	do {
		...
		if (update) {        // Update weight & volume information
			...
			mvwprintz(w_pickup, 0, 9, (new_weight >= u.weight_capacity() * .25 ? c_red : c_white), "Wgt %d", new_weight);
			wprintz(w_pickup, c_white, "/%d", u.weight_capacity());
			mvwprintz(w_pickup, 0, 22, (new_volume > u.volume_capacity() - 2 ? c_red : c_white), "Vol %d", new_volume);
...
void game::throw_item(player &p, int tarx, int tary, item &thrown, std::vector<Point> &trajectory) {
	...
	std::string message;
	int dam = (thrown.weight() + thrown.type->melee_dam + p.str_cur) / double(2 + double(thrown.volume() / 6));
	int i, tx, ty;
	...
	for (i = 0; i < trajectory.size() && dam > 0; i++) {
		...
		if (mon_at(tx, ty) != -1 && (!missed || one_in(7 - int(z[mon_at(tx, ty)].type->size)))) {
			...
			if (!p.is_npc())
				add_msg("%s You hit the %s for %d damage.", message.c_str(), z[mon_at(tx, ty)].name().c_str(), dam);
...
void game::plfire(bool burst) {
	...
	// target() sets x and y, or returns false if we canceled (by pressing Esc)
	std::vector<Point> trajectory = target(x, y, x0, y0, x1, y1, mon_targets, passtarget, &u.weapon);
...
void game::fire(player &p, int tarx, int tary, std::vector<Point> &trajectory, bool burst) {
	...
	switch (firing->skill_used) {
		...
		case (sk_rifle):
			...
			break;
		default:
			debugmsg("Why is shooting %s using %s skill?", (firing->name).c_str(), skill_name(firing->skill_used).c_str());
	}
	...
	for (int curshot = 0; curshot < num_shots; curshot++) {
		...
		missed = false;
		if (debugmon)
			debugmsg("missed_by %f deviation %f trange %d charges %d myx %d myy %d monx %d mony %d",
		...
		for (int i = 0; i < trajectory.size() && dam > 0; i++) {
			...
			if (u_see(trajectory[i].x, trajectory[i].y, junk)) {
				mvwputch(w_terrain, trajectory[i].y + SEEY - u.posy, trajectory[i].x + SEEX - u.posx, c_red, '`');
				wrefresh(w_terrain);
				nanosleep(&ts, NULL);
			}

			if (dam <= 0) {
				if (is_bolt && ((u.weapon.curammo->m1 == WOOD && !one_in(5)) || (u.weapon.curammo->m1 != WOOD && !one_in(15))))
			...
			if (mondex != -1 && (!z[mondex].has_flag(MF_DIGS) || rl_dist(p.posx, p.posy, z[mondex].posx, z[mondex].posy) <= 1) &&
				...
				if (z[mondex].has_flag(MF_HARDTOSHOOT) && !one_in(6) &&
					...
					if (u_see_mon)
						add_msg("The shot passes through the %s without hitting.", z[mondex].name().c_str());
					goodhit = 1;
				}
				else {
					...
					if (dam > 0) {
						...
						if (&p == &u && u_see_mon)
							add_msg("%s You hit the %s for %d damage.", message.c_str(), z[mondex].name().c_str(), dam);
						else if (u_see_mon)
							add_msg("%s %s shoots the %s.", message.c_str(), p.name.c_str(), z[mondex].name().c_str());
			...
			else if ((!missed || one_in(5)) && (npc_at(tx, ty) != -1 || u.posx == tx && u.posy == ty)) {
				double goodhit = missed_by;
				body_part hit;
				player *h;
				...
				if (dam > 0) {
					if (h == &u) {
						add_msg("%s %s shoots your %s!", message.c_str(), p.name.c_str(), body_part_name(hit, side).c_str());
					}
					else {
						h->moves -= dam * 4;
						if (hit == bp_legs)
						...
						if (&p == &u)
							add_msg("%s You hit %s's %s for %d damage.", message.c_str(), h->name.c_str(), body_part_name(hit, side).c_str(), dam);
						else if (u_see(tx, ty, junk))
							add_msg("%s %s shoots %s's %s for %d damage.", message.c_str(), p.name.c_str(), h->name.c_str(), body_part_name(hit, side).c_str(), dam);
					}
...
void game::butcher() {
	...
	// We do it backwards to prevent the deletion of a corpse from corrupting our vector of indices.
			...
			for (int j = 0; j < group; j++) {    // For each monster in the group...
				...
				if (type == mon_null)
					...
					do {
						monx = rng(0, SEEX * 3 - 1);
						mony = rng(0, SEEY * 3 - 1);
						if (shiftx == -1) monx = 0 - SEEX * 2;
						if (shiftx == 1) monx = SEEX * 4;
						if (shifty == -1) mony = 0 - SEEX * 2;
						if (shifty == 1) mony = SEEY * 4;
						monx += rng(-5, 10);
						mony += rng(-5, 10);
						iter++;
...
std::vector<Point> game::target(int &x, int &y, int lowx, int lowy, int hix, int hiy, std::vector<monster> t, int &target, item *relevent) {
...
overmap_terrain_id game::ter_at(int omx, int omy, bool &mark_as_seen) {
	overmap_terrain_id ret;
	int sx = 0, sy = 0;
	if (omx >= OMAPX)
		sx = 1;
	if (omx < 0)
...
void intro() {
	...
	if (maxy < 25 || maxx < 80) {
		endwin();
		erase();
		printf("\nThis game requires a minimum terminal size of 80x25!\n");
		exit(0);
	}
}
