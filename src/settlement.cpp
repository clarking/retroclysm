...
int settlement::num(overmap_terrain_id ter) {
	if (ter < ot_set_house || ter > ot_wall) {
		debugmsg("settlement::num requested back overmap_terrain_id (%s)", oterlist[ter].name.c_str());
		return -1;
	}
	return buildings[ter - ot_set_house];
}

void settlement::add_building(overmap_terrain_id ter) {
	if (ter < ot_set_house || ter > ot_wall)
		return;
